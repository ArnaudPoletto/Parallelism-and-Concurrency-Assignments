...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd.aggregate(0)((acc, art) => if (art.text.split(" ").contains(lang)) acc + 1 else acc , _ + _)
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    //rdd.flatMap(art => langs.filter(l => art.text.split(" ").contains(l)).map(l => (l, art))).groupByKey()
    for {
      art <- rdd
      l <- langs
      if art.text.split(" ").contains(l)
    } yield (l, art)
  }.groupByKey()
