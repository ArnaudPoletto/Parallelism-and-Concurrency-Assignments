...
package object barneshut {
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body]) extends Quad {
    def mass = bodies.foldLeft(0f)((sum, body) => sum + body.mass)
    def massX = bodies.foldLeft(0f)((sum, body) => sum + body.mass * body.x) / mass
    def massY = bodies.foldLeft(0f)((sum, body) => sum + body.mass * body.y) / mass
    ...
    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        val tmp: Fork = Fork(
          ...
          Empty(centerX - 0.25f * size, centerY + 0.25f * size, size / 2f),
          Empty(centerX + 0.25f * size, centerY + 0.25f * size, size / 2f))
        bodies.foldLeft(tmp)(_ insert _).insert(b)
      }
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      // body pos + bound + sector precision
      /*
      val x: Int = math.floor((boundaries.maxX - boundaries.minX) / sectorPrecision).toInt
      val y: Int = math.floor((boundaries.maxY - boundaries.minY) / sectorPrecision).toInt
      */
